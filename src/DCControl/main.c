...
int main(int argc, char const *argv[])
{
  ...
  while (true) {
    ...
    // abort <time>
      ...
      pid_t abort_pid = fork(); //creamos un hijo para que haga el abort en paralelo y no se bloquee el main
      if (abort_pid == 0) {
        ...
        //proceso padre, no hace nada, solo espera a que el hijo termine
        continue;
      } else {
        perror("fork falló");
        }
